...
public class BSON {
	...
	public static void serialize(Object o, StringBuilder buf) {
		BSONSerializers.getLegacy().serialize(o, buf);
	}
	...
	public static Object parse(String s, BSONCallback c) {
		...
		BSONParser p = new BSONParser(s, c);
		return p.parse();
	}
...
class BSONParser {
	 ...
	 * Create a new parser.
	 */
	public BSONParser(String s, BSONCallback callback) {
		this.s = s;
	 ...
	 */
	public void readHex() {
		if (pos < s.length() && ((s.charAt(pos) >= '0' && s.charAt(pos) <= '9') || (s.charAt(pos) >= 'A' && s.charAt(pos) <= 'F') || (s.charAt(pos) >= 'a' && s.charAt(pos) <= 'f'))) {
			pos++;
		} else {
			throw new JSONParseException(s, pos);
		}
